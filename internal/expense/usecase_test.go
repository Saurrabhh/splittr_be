...
func (m *mockExpenseRepository) CreateExpense(ctx context.Context, e *expense.Expense) error {
	return m.Called(ctx, e).Error(0)
}

func (m *mockExpenseRepository) CreateExpenseSplit(ctx context.Context, s *expense.Split) error {
	return m.Called(ctx, s).Error(0)
}

func (m *mockExpenseRepository) GetExpenseByID(ctx context.Context, id string) (*expense.Expense, error) {
...
func (m *mockExpenseRepository) ListExpenseSplits(ctx context.Context, expenseID string) ([]expense.Split, error) {
...
func (m *mockExpenseRepository) ListExpensesByGroup(ctx context.Context, groupID string, limit int32, lastTime *time.Time, lastID *string) ([]expense.Expense, error) {
...
func (m *mockExpenseRepository) ListUserPersonalExpenses(ctx context.Context, userID string, limit int32, lastTime *time.Time, lastID *string) ([]expense.Expense, error) {
...
func (m *mockExpenseRepository) ListUserFriendExpenses(ctx context.Context, userID string, limit int32, lastTime *time.Time, lastID *string) ([]expense.Expense, error) {
	args := m.Called(ctx, userID, limit, lastTime, lastID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func (m *mockExpenseRepository) GetGroupBalances(ctx context.Context, groupID string) ([]expense.UserBalance, error) {
	args := m.Called(ctx, groupID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
	return args.Get(0).([]expense.UserBalance), args.Error(1)
}

func (m *mockExpenseRepository) GetFriendBalances(ctx context.Context, userID string) ([]expense.UserBalance, error) {
...
func (m *mockExpenseRepository) GetGroupPairwiseDebts(ctx context.Context, groupID string) ([]expense.PairwiseDebt, error) {
	args := m.Called(ctx, groupID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func TestCreateExpense_Success_EqualSplit(t *testing.T) {
	...
	exp, splits, err := uc.CreateExpense(ctx, "Dinner", 100.0, "INR", "Food", nil, paidBy, expense.SplitTypeEqual, inputs, creatorID)
...
func TestCreateExpense_Success_GroupExactSplit(t *testing.T) {
	...
	uc := expense.NewUseCase(mockRepo, mockTx, mockGroupSvc, mockAct, mockNotif)
	exp, splits, err := uc.CreateExpense(ctx, "Hotel", 100.0, "INR", "Travel", &groupID, paidBy, expense.SplitTypeExact, inputs, creatorID)
...
func TestCreateExpense_Success_PercentageSplit(t *testing.T) {
	...
	uc := expense.NewUseCase(mockRepo, mockTx, mockGroupSvc, mockAct, mockNotif)
	exp, splits, err := uc.CreateExpense(ctx, "Party", 200.0, "USD", "Entertainment", nil, paidBy, expense.SplitTypePercentage, inputs, creatorID)
...
func TestCreateExpense_ValidationErrors(t *testing.T) {
	...
	// Empty description
	_, _, err := uc.CreateExpense(ctx, "", 100.0, "INR", "Cat", nil, "usr-1", expense.SplitTypeEqual, []expense.InputSplit{{UserID: "usr-1"}}, "usr-1")
	...
	_, _, err = uc.CreateExpense(ctx, "Desc", 0.0, "INR", "Cat", nil, "usr-1", expense.SplitTypeEqual, []expense.InputSplit{{UserID: "usr-1"}}, "usr-1")
	require.ErrorAs(t, err, &appErr)
	assert.Equal(t, response.TypeValidation, appErr.Type)
	assert.Contains(t, appErr.Message, "amount must be greater than zero")

	// Empty splits inputs
	_, _, err = uc.CreateExpense(ctx, "Desc", 100.0, "INR", "Cat", nil, "usr-1", expense.SplitTypeEqual, nil, "usr-1")
	require.ErrorAs(t, err, &appErr)
	assert.Equal(t, response.TypeValidation, appErr.Type)
	...
	amt1 := 50.0
	amt2 := 40.0
	inputsMismatch := []expense.InputSplit{
		{UserID: "usr-1", Amount: &amt1},
		{UserID: "usr-2", Amount: &amt2},
	}
	_, _, err = uc.CreateExpense(ctx, "Desc", 100.0, "INR", "Cat", nil, "usr-1", expense.SplitTypeExact, inputsMismatch, "usr-1")
	require.ErrorAs(t, err, &appErr)
	assert.Equal(t, response.TypeValidation, appErr.Type)
	assert.Contains(t, appErr.Message, "does not match total expense amount")

	// Exact split missing amount
	inputsMissingAmt := []expense.InputSplit{
		{UserID: "usr-1"},
	}
	_, _, err = uc.CreateExpense(ctx, "Desc", 100.0, "INR", "Cat", nil, "usr-1", expense.SplitTypeExact, inputsMissingAmt, "usr-1")
	require.ErrorAs(t, err, &appErr)
	assert.Equal(t, response.TypeValidation, appErr.Type)
	...
	p1 := 50.0
	p2 := 40.0
	inputsPctMismatch := []expense.InputSplit{
		{UserID: "usr-1", Percentage: &p1},
		{UserID: "usr-2", Percentage: &p2},
	}
	_, _, err = uc.CreateExpense(ctx, "Desc", 100.0, "INR", "Cat", nil, "usr-1", expense.SplitTypePercentage, inputsPctMismatch, "usr-1")
	require.ErrorAs(t, err, &appErr)
	assert.Equal(t, response.TypeValidation, appErr.Type)
	...
	_, _, err = uc.CreateExpense(ctx, "Desc", 100.0, "INR", "Cat", &groupID, "usr-1", expense.SplitTypeEqual, []expense.InputSplit{{UserID: "usr-2"}}, "usr-1")
	require.ErrorAs(t, err, &appErr)
	assert.Equal(t, response.TypeValidation, appErr.Type)
	...
	_, _, err = uc.CreateExpense(ctx, "Desc", 100.0, "INR", "Cat", &groupID, "usr-1", expense.SplitTypeEqual, []expense.InputSplit{{UserID: "usr-non-member"}}, "usr-1")
	require.ErrorAs(t, err, &appErr)
	assert.Equal(t, response.TypeValidation, appErr.Type)
...
func TestCreateExpense_TransactionFailure(t *testing.T) {
	...
	uc := expense.NewUseCase(mockRepo, mockTx, mockGroupSvc, mockAct, mockNotif)
	_, _, err := uc.CreateExpense(ctx, "Dinner", 100.0, "INR", "Food", nil, "usr-1", expense.SplitTypeEqual, []expense.InputSplit{{UserID: "usr-1"}}, "usr-1")
...
func TestSettleUp_ValidationErrors(t *testing.T) {
	...
	uc := expense.NewUseCase(mockRepo, mockTx, mockGroupSvc, nil, nil)
...
func TestGetExpenseDetails_Success(t *testing.T) {
	...
	expID := "exp-1"
	userID := "usr-1"
	expectedExp := &expense.Expense{ID: expID, Description: "Lunch", PaidBy: userID, CreatedBy: userID}
	expectedSplits := []expense.Split{{ExpenseID: expID, UserID: userID, Amount: 30.0}}
...
func TestGetExpenseDetails_NotFound(t *testing.T) {
	...
	expID := "non-existent"
	mockRepo.On("GetExpenseByID", ctx, expID).Return((*expense.Expense)(nil), nil)
...
func TestGetExpenseDetails_Forbidden(t *testing.T) {
	...
	expID := "exp-1"
	expectedExp := &expense.Expense{ID: expID, Description: "Lunch", PaidBy: "usr-creator", CreatedBy: "usr-creator"}
	expectedSplits := []expense.Split{{ExpenseID: expID, UserID: "usr-other", Amount: 30.0}}
...
func TestDeleteExpense_Creator_Success(t *testing.T) {
	...
	e := &expense.Expense{ID: expID, CreatedBy: creatorID}
